...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        input_channels = num_input_channels # for first layer
        output_width = 0
        for i in range(self.nlayers):
            self.convolutional_layers.append(
                Conv1d(in_channels=input_channels, out_channels=num_channels[i], kernel_size=kernel_sizes[i], stride=strides[i], weight_init_fn=conv_weight_init_fn, bias_init_fn=bias_init_fn)
            )
            # for other layers
        ...
        in_features = num_channels[-1] * output_width
        self.linear_layer = Linear(in_features=in_features, out_features=num_linear_neurons)
    ...
    def backward(self, labels):
        ...
        for i in reversed(range(self.nlayers)):
            ...
            dLdA = self.convolutional_layers[i].backward(dLdA * dAdZ)

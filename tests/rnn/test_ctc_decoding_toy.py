...
from mytorch.nn.CTCDecoding import BeamSearchDecoder
...
class BeamSearchToyTest(Test):
    ...
    def test_beam_search_toy(self):
        ...
        try:
            assert list(MergedPathScores.keys()) == list(
                expected_MergedPathScores.keys()
            )
            for key in MergedPathScores.keys():
                assert np.allclose(
                    np.array(MergedPathScores[key]), expected_MergedPathScores[key]
                )
        except AssertionError:
            print(
                f"Incorrect Merged Path Scores\nExpected: {expected_MergedPathScores}\nPredicted: {MergedPathScores}"
            )
            return False
        else:
            print(
                f"Correct Merged Path Scores\nExpected: {expected_MergedPathScores}\nPredicted: {MergedPathScores}"
            )

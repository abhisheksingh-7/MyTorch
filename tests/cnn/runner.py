...
import models.mlp_scan as cnn_solution
import models.cnn
...
def test_upsampling_1d_correctness():
    ...
    expected_res = np.load(
        "autograder/ref_result/upsample_1d_res.npz", allow_pickle=True
    )
...
def test_downsampling_1d_correctness():
    ...
    expected_res = np.load(
        "autograder/ref_result/downsample_1d_res.npz", allow_pickle=True
    )
...
def test_upsampling_2d_correctness():
    ...
    expected_res = np.load(
        "autograder/ref_result/upsample_2d_res.npz", allow_pickle=True
    )
...
def test_downsampling_2d_correctness():
    ...
    expected_res = np.load(
        "autograder/ref_result/downsample_2d_res.npz", allow_pickle=True
    )
...
def conv2d_correctness_stride1():
    ...
    test_model = Conv2d_stride1(
        in_c, out_c, kernel, random_normal_weight_init_fn, np.zeros
    )
...
def conv2d_correctness():
    ...
    test_model = Conv2d(
        in_c, out_c, kernel, stride, random_normal_weight_init_fn, np.zeros
    )
...
def test_convTranspose_1d_correctness():
    ...
    expected_res = np.load(
        "autograder/ref_result/convTranspose_1d_res.npz", allow_pickle=True
    )

    try:
        for i in range(3):
            assert (
                expected_res["conv_forward_res_list"][i].shape == ep_fres_arr[i].shape
            )
            assert (
                expected_res["conv_backward_res_list"][i].shape == ep_bres_arr[i].shape
            )
            assert np.allclose(expected_res["conv_forward_res_list"][i], ep_fres_arr[i])
            assert np.allclose(
                expected_res["conv_backward_res_list"][i], ep_bres_arr[i]
            )
...
def test_convTranspose_2d_correctness():
    ...
    expected_res = np.load(
        "autograder/ref_result/convTranspose_2d_res.npz", allow_pickle=True
    )
...
def test_MaxPool2d_stride1_correctness():
    ...
    expected_res = np.load(
        "autograder/ref_result/maxpool2d_stride1_res.npz", allow_pickle=True
    )

    try:
        for i in range(3):
            assert (
                expected_res["maxpool_forward_res_list"][i].shape
                == ep_fres_arr[i].shape
            )
            assert (
                expected_res["maxpool_backward_res_list"][i].shape
                == ep_bres_arr[i].shape
            )
            assert np.allclose(
                expected_res["maxpool_forward_res_list"][i], ep_fres_arr[i]
            )
            assert np.allclose(
                expected_res["maxpool_backward_res_list"][i], ep_bres_arr[i]
            )
...
def test_simple_scanning_mlp():
    data = np.loadtxt(os.path.join("autograder", "data", "data.asc")).T.reshape(
        1, 24, -1
    )
    cnn = cnn_solution.CNN_SimpleScanningMLP()
    weights = np.load(
        os.path.join("autograder", "weights", "mlp_weights_part_b.npy"),
        allow_pickle=True,
    )
    cnn.init_weights(weights)

    expected_result = np.load(
        os.path.join("autograder", "ref_result", "res_b.npy"), allow_pickle=True
    )
...
def test_distributed_scanning_mlp():
    ...
    weights = np.load(
        os.path.join("autograder", "weights", "mlp_weights_part_c.npy"),
        allow_pickle=True,
    )
    cnn.init_weights(weights)

    expected_result = np.load(
        os.path.join("autograder", "ref_result", "res_c.npy"), allow_pickle=True
    )
    result = cnn.forward(data)
...
def get_cnn_model():
    ...
    model.linear_layer.W = linear_weight_init_fn(
        model.linear_layer.W.shape[0], model.linear_layer.W.shape[1]
    )
...
def cnn_model_correctness(idx):
    ...
    labels = np.load(
        os.path.join("autograder", "data", "labels.npy"), allow_pickle=True
    )
